...
int main(int argc, char** argv) {
  ...
  FortranMatrix<double> A, B, C, x;
  if (my_rank == 0) {
    // A = get_double_random_matrix(A_rows, A_cols, 0, 6);
    A = get_double_matrix_10_i_j_1(A_rows, A_cols);
    // B = get_double_random_matrix(B_rows, B_cols, 0, 6);
    B = get_double_matrix_10_i_j_1(B_rows, B_cols);
    cout << "Matrix A:" << endl;
    A.print();
    cout << "Matrix B:" << endl;
    B.print();
  ...
  pdgemm_(&N, &N, &A_rows, &B_cols, &A_cols, &alpha, localA.data(), &iONE,
